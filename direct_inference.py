...
parser.add_argument('--use_swin_attention', type=str2bool, default=False)
...
# exit()
##使用window attention
use_swin_attention = args.use_swin_attention
if use_swin_attention:
    method_name = 'swin_attention'
else:
    method_name = ''
...
if use_v_res:
    ...
    method_name = method_name+'_add_-'+str(weight_vres)+'crosstimex0'
...
method_name = method_name
control_params={}
control_params['use_swin_attention'] = use_swin_attention
...
method_name = '_nox0_low_guidancedirect_inference'
...
for index, prompt in enumerate(prompts[iter_id*iter_num:(iter_id+1)*iter_num]):
    ...
    if not valid:
        continue
    # if not file_id == '7046163263156891738':
        # continue
